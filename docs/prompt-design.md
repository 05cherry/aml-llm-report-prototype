...
- 위험도는 모델 출력을 그대로 신뢰하지 않는다. 코드(`src/signals.py`의 `derive_risk`)가
  `docs/aml-signals.md`의 위험도 표(S4 포함 또는 신호 2개 이상이면 high, 단일 신호면
  medium, 없으면 none)를 그대로 적용해 파생한 값을 리포트의 위험도로 렌더링한다.
  모델이 제출한 `risk_level`은 스키마상 필수로 받지만, 파생값과 어긋나면 파생값이
  우선한다(문서 우선). 이로써 모델이 신호는 맞게 잡고 위험도만 틀리는 경우를 차단한다.
